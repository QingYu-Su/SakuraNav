...
- **标签编辑双 Tab** — 标签编辑弹窗支持基本信息与关联网站双 Tab 布局
...
- **备忘便签** — 为网站添加备注和待办列表，Todo 角标显示，支持 AI 可读
- **关联推荐** — 基于 AI 分析推荐相关站点，推荐内容参与站内搜索
- **右键菜单** — 网站卡片支持右键快捷操作
- **备选 URL** — 为网站设置备选访问地址，支持条件自动切换
- **站点级在线检测** — 每个网站可独立配置在线检测参数（超时、判定模式、连续失败阈值）
- **重复 URL 检测** — 创建网站时自动检测重复 URL 并提示
- **自定义 Tooltip** — 悬浮提示使用自建组件，体验更佳
...
- **仅卡片导出/导入** — 支持仅导出/导入网站卡片数据，HMAC 签名校验完整性

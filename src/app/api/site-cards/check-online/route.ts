...
import { getOnlineCheckSiteCards, updateCardsOnlineStatus, sendNotificationToUser, getAppSettings } from "@/lib/services";
...
async function sendOfflineNotifications(changes: OnlineStatusChange[]): Promise<void> {
  ...
  for (const [ownerId, siteChanges] of grouped) {
    for (const change of siteChanges) {
      ...
      const content = `网站卡片「${change.cardName}」(${change.cardUrl}) 当前处于离线状态，请及时检查。`;
...
export async function POST(request: Request) {
  try {
    ...
    const cards = await getOnlineCheckSiteCards();
    if (cards.length === 0) {
    ...
    if (cardsToCheck.length > 0) {
      for (let i = 0; i < cardsToCheck.length; i += CONCURRENCY) {
    ...
    logger.info(`批量检查完成: 共 ${allResults.size} 个卡片, ${offlineChanges.length} 个状态变更`);

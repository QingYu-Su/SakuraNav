/**
 * AI 网站分析 API（统一入口）
 * @description 通过 AI 分析网站，支持两种范围：
 *   scope: "basic" — 仅分析基本信息（标题、描述、标签）
 *   scope: "full"  — 全部分析（基本信息 + 推荐上下文 + 关联网站）
 *
 * 全部分析模式下，一次 AI 调用同时返回推荐上下文和关联网站，无需分开请求。
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.info("开始 AI 分析网站", { url, scope });

    const model = createLanguageModel(config);
    const result = await generateText({ model, prompt: buildPrompt(url, tagList, scope, candidateSites, targetSite) });
    ...
    if (scope === "full") {
      ...
      const recs = Array.isArray(parsed.recommendations)
        ...
        : [];
      // 只保留候选列表中实际存在的 siteId
      const validIds = new Set(candidateSites.map((s) => s.id));
      response.recommendations = recs.filter((r) => validIds.has(r.siteId));
    }
...
5. 只返回 JSON，不要有其他内容`;
  ...
  // scope === "full"
  const contextHint = targetSite?.recommendContext?.trim()
    ? `\n- 推荐上下文（用户提供，辅助参考）：${targetSite.recommendContext}`
    : "";
  const siteName = targetSite?.name ?? "";

  const siteListSection = candidateSites.length > 0
    ? `\n以下是导航站中的其他网站列表：\n${candidateSites.map((s) => `ID: ${s.id} | 名称: ${s.name} | URL: ${s.url} | 描述: ${s.description} | 标签: ${s.tags.join(", ")}`).join("\n")}`
    : "\n（导航站中暂无其他网站）";

  return `你是一个综合分析助手。请分析以下网站 URL，同时完成三项任务：(A) 基本信息、(B) 推荐上下文、(C) 关联网站。

网站 URL: ${url}${siteName ? `\n网站名称：${siteName}` : ""}${contextHint}

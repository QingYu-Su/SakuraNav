 ...
 * 支持两种导出范围：full（全部卡片）和 sites-only（仅网站卡片）
...
function buildExportFilename(username: string, scope: string) {
  ...
  const scopeSuffix = scope === "sites-only" ? "-sites" : "";
  return `sakura-${username}${scopeSuffix}-${parts.join("")}.zip`;
}

/**
 * 导出当前用户的数据
 * @query scope - "full"（全部卡片）或 "sites-only"（仅网站卡片），默认 "full"
 */
export async function POST(request: Request) {
  try {
    ...
    const scope: "full" | "sites-only" =
      rawScope === "sites-only" ? rawScope : "full";
    const sitesOnly = scope === "sites-only";

    // 使用可扩展的数据收集服务
    const exportData = await collectExportData(ownerId, sitesOnly);

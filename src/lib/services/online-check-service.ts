...
export async function performSingleSiteCardOnlineCheck(cardId: string): Promise<boolean | null> {
  ...
  const cachedOnline = await getUrlOnlineStatusIfFresh(card.url);
  if (cachedOnline !== null) {
    const statusChange = await updateCardOnlineStatus(card.id, cachedOnline);
    logger.info(`${cachedOnline ? "✓" : "✗"} ${card.url}（使用缓存结果）`);
    if (statusChange?.wentOffline) {
      const settings = await getAppSettings();
      const appName = settings.siteName || "SakuraNav";
      sendNotificationToUser(statusChange.ownerId, `${appName} 站点离线通知`, `网站「${card.name}」(${card.url}) 当前处于离线状态，请及时检查。`).catch(() => {});
    }
    return cachedOnline;
  }
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    online = await checkSiteOnline(card.url);
    if (online) {
      if (attempt > 1) {
        logger.info(`✓ ${card.url}（第 ${attempt} 次重试成功）`);
      } else {
        logger.info(`✓ ${card.url}（单站点检测成功）`);
      }
      break;
    }
    if (attempt < MAX_ATTEMPTS) {
      logger.info(`✗ ${card.url}（检测失败，第 ${attempt} 次重试...）`);
      await delay(RETRY_DELAY_MS);
    }
  }

  if (!online) {
    logger.info(`✗ ${card.url}（重试 ${MAX_ATTEMPTS} 次后仍离线）`);
  }

  // 3. 更新 URL 缓存
  await upsertUrlOnlineCache(card.url, online);
  ...
  if (statusChange?.wentOffline) {
    const settings = await getAppSettings();
    const appName = settings.siteName || "SakuraNav";
    sendNotificationToUser(statusChange.ownerId, `${appName} 站点离线通知`, `网站「${card.name}」(${card.url}) 当前处于离线状态，请及时检查。`).catch((err) => {

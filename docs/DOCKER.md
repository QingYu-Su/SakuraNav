...
Docker 构建时会自动检测服务器可用资源并限制峰值负载：

- **内存**：读取 `/proc/meminfo` 可用内存，预留 512MB 给系统，Node.js 堆上限取剩余的 60%（范围 1024MB ~ 4096MB）
- **CPU**：Next.js 构建 worker 数量限制为 `min(CPU核心数, 2)`，防止多核服务器并行编译导致 CPU 飙升

...
│   │   │   ├── note-card-editor.tsx       # 笔记卡片编辑器（标题 + Markdown 内容，支持列表自动续行、剪贴板粘贴图片）
...
| 图片上传 | `app/api/cards/note/upload-image/route.ts` | 剪贴板粘贴图片上传，kind=`note-image` |
| 图片访问（伪装 URL） | `app/api/cards/note/img/[imageId]/route.ts` | `/api/cards/note/img/{id}` 不暴露内部 asset 路径 |
| 孤立图片清理 | `app/api/cards/note/route.ts` → `cleanupOrphanNoteImages()` | POST/PUT/DELETE 后异步清理未引用的 `note-image` 资源 |
>
> **关键约定**：社交卡片过滤条件已统一为 `card_type IS NOT NULL AND card_type != 'note'`，新增卡片类型时需同步更新此过滤条件以避免误包含。

> 💡 **card_data asset 引用映射** — `remapCardDataAssets()` 通用扫描 JSON payload 中的 asset 引用并映射。社交卡片使用 `/api/assets/{id}/file` 格式（如 `qrCodeUrl`），笔记卡片使用 `/api/cards/note/img/{id}` 格式（markdown 中的图片 URL，支持一张笔记内多张图片）。适用于任何卡片类型的 payload 结构。
...
| `POST` | `/api/cards/note/upload-image` | 上传笔记图片（需认证，FormData） |

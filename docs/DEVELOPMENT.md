...
**撤销触发方式**：
1. **Ctrl+Z 快捷键** — `sakura-nav-app.tsx` 监听 `keydown`，调用 `undoStack.pop()` 执行最近的 undo 回调
...
> 💡 `siteToFormState()`（`components/admin/types.ts`）是 Site → `SiteFormState` 的 **唯一映射入口**。新增 Site 字段时，只需在此函数中添加映射，所有撤销逻辑（创建/编辑/删除快照）会自动跟随，无需手动同步其他位置。

...
│   │   │   ├── note-card-editor.tsx       # 笔记卡片编辑器（内容编辑/预览，支持列表自动续行、剪贴板粘贴、/ 快捷指令、独立 Ctrl+Z 撤回）
...
│   │   └── use-snapshots.ts          # 快照管理（创建/列表/恢复/删除/重命名 + 编辑模式追踪 + 页面卸载保存）
│   │
...
CREATE TABLE assets (
  ...
  original_name TEXT,                  -- 原始文件名（笔记附件等场景）
  note_id TEXT,                        -- 关联的笔记卡片 ID（仅笔记附件）
...
| `logo` | Logo |
| `favicon` | Favicon |
| `note-image` | 笔记内联图片（编辑器粘贴或 / 指令，≤10MB） |
| `note-file` | 笔记内联文件（编辑器粘贴或 / 指令，≤100MB） |
| `note-attachment` | 笔记大文件附件（历史兼容，通过 `note_id` 关联笔记） |
...
CREATE TABLE snapshots (
  id TEXT PRIMARY KEY,                 -- 快照ID (snap-UUID)
  owner_id TEXT NOT NULL,              -- 数据所有者 ID
  label TEXT NOT NULL,                 -- 快照标签（如 v1, v2）
  data TEXT NOT NULL,                  -- 快照数据 JSON（含 tags/sites/site_tags/theme_appearances/app_settings/site_relations/floating_buttons）
  created_at TEXT NOT NULL             -- 创建时间 (ISO 8601)
);
```
...
>
> 💡 **防重复创建**: 使用 `hasEditedRef` 追踪编辑状态，`sendBeacon` 用于页面卸载场景，`hasEditedRef.current = false` 在 `await` 前重置以防止 React Strict Mode 竞态。
...
```typescript
function createAsset(input: { filePath, mimeType, kind, originalName?, noteId?, fileSize? }): { id, kind, url }
function getAsset(id: string): StoredAsset | null
function listStoredAssets(): StoredAsset[]
function deleteAsset(id: string): void
function getAssetsByKind(kind: string): StoredAsset[]

// 笔记附件管理
function getNoteAttachments(noteId: string): StoredAsset[]        // 获取指定笔记的所有附件
function renameAssetOriginalName(id: string, newName: string): void // 重命名附件
function associateAssetsWithNote(assetIds: string[], noteId: string): void // 批量关联附件到笔记
function unlinkAssetsFromNote(assetIds: string[]): void             // 软删除：将 note_id 置空（支持撤销恢复）
function getAssetsByNoteId(noteId: string): StoredAsset[]          // 获取指定笔记的所有资源（用于删除清理）
function deleteAssetsByNoteId(noteId: string): void                // 删除指定笔记的所有资源记录

// 孤立资源检测
function findOrphanNoteAssets(referencedAssetIds: Set<string>): StoredAsset[] // 检测未引用的内联资源 + 未关联笔记的附件
```
...
<details>
<summary><strong>SnapshotRepository</strong> — 快照数据访问（<code>snapshot-repository.ts</code>）</summary>

```typescript
// 读取
function getSnapshotMetas(ownerId: string): SnapshotMeta[]         // 获取快照元信息列表（按时间倒序）
function getSnapshotById(id: string, ownerId: string): (SnapshotMeta & { data: SnapshotData }) | null
...
function deleteSnapshotsAfter(ownerId: string, snapshotId: string): number  // 删除指定快照之后的所有快照
function cleanupExpiredSnapshots(): number                                  // 清理超过 30 天的快照

// 恢复
function restoreFromSnapshot(id: string, ownerId: string): boolean  // 事务内清空并恢复卡片和标签数据，外观和设置不受影响，删除后续快照

// 重命名
function renameSnapshot(id: string, ownerId: string, label: string): boolean
```
...
| DataPortabilityService | `data-portability-service.ts` | 用户数据可移植性：可扩展导出（表白名单+列黑名单，支持全部卡片/仅网站卡片，含 `site_relations` 关联推荐导出）、clean/增量/覆盖导入（含 `site_relations` 映射重建）、HMAC-SHA256 签名校验防篡改 |
...
| 约定 | 说明 |
|:-----|:-----|
| `TodoItem.noteId` | `types.ts` 中的可选字段。有值 = 笔记引用自动生成的 todo，无值 = 用户手动添加。只需在类型中维护此字段，所有判断逻辑自动跟随 |
| `siteInputSchema` + `memoUpdateSchema` | 两处 Zod schema 均已包含 `noteId: z.string().optional()`，确保 API 保存时不会丢失该字段 |
| `syncSiteTodosFromNotes()` | 在 `route.ts` 中每次笔记 CRUD 后调用。自动：添加新引用的 todo、移除不再引用的 todo、保留完成状态和用户手动 todo |
| `getAllNormalSiteTodos()` | `site-repository.ts` 中的 Repository 函数，查询所有普通网站的 todos 用于清理。新增类似批量查询需求时只需在此 Repository 中添加函数 |
| UI 区分 | 笔记引用的 todo 使用 indigo 底色，用户手动的使用默认色。在 `site-memo-dialogs.tsx` 和 `notes-tab.tsx` 中通过 `todo.noteId` 判断样式 |
...
**轻量刷新策略**：编辑保存后的数据刷新按操作类型分级处理，避免不必要的全量 API 请求导致所有卡片闪烁：

| 场景 | 刷新方式 | 说明 |
|:-----|:---------|:-----|
| 网站卡片 PUT（编辑） | `updateSiteInCache()` 就地更新 | 仅替换 `siteList.items` 和 `adminData.sites` 中对应条目 |
| 网站卡片编辑后标签关联变化 | `syncNavigationData()` | 需要刷新标签列表以更新 `siteCount` |
| 网站卡片编辑后在线检测完成 | `updateSiteOnlineStatusInCache()` 就地更新 | 仅更新 `isOnline` 字段，不触发全量刷新 |
| 社交/笔记卡片 PUT（编辑） | 本地 `setCards(prev => prev.map(...))` 就地更新 | 仅更新 cards 数组中对应条目 |
| 笔记查看弹窗 checkbox 交互 | `consumeViewModified()` 消费标记，仅在变更时 `syncNavigationData()` | 无变更则不刷新 |
| 新建/删除/撤销 | `syncNavigationData()` + `syncAdminBootstrap()` 全量刷新 | 排序位置、标签关联等需完整同步 |

> 💡 **可扩展性约定** — `updateSiteInCache` 和 `updateSiteOnlineStatusInCache` 定义在 `use-sakura-nav-orchestrator.ts` 中，同时更新 `siteList.items`（前台展示）和 `adminData.sites`（管理面板）两份数据。新增需要就地更新的字段时，只需在对应 hook 中调用这些方法即可，无需修改 orchestrator。

**笔记附件延迟持久化**：附件 Tab 的操作（上传/删除）在编辑模式下仅标记，不立即持久化：
1. 上传附件时不关联 `noteId`（保持 `note_id = null`），统一通过 `pendingAttachmentIds` 跟踪
2. 删除附件仅标记到 `deletedAttachmentIds`，从本地列表移除但不调用 API
3. 点击「保存」时统一处理：`attachmentIds` 关联新附件 + `softDeleteAttachmentIds` 软删除标记的附件
4. 撤销操作通过反向附件变更实现：将软删除的附件重新关联，将新增的附件解除关联
5. 取消编辑时，未关联的孤立附件会在后续笔记操作时被 `findOrphanNoteAssets` 自动回收
...
> | 编辑器独立撤回 | `components/sakura-nav/note-card-editor.tsx` | Ctrl+Z 独立撤回栈（与全局撤回系统无关，最多 30 条，500ms 防抖） |
| 附件下载（伪装 URL，兼容旧数据） | `app/api/cards/note/attach/[attachId]/route.ts` | `/api/cards/note/attach/{id}` 原始文件名下载，Content-Disposition |
| 查看弹窗附件渲染 | `components/dialogs/note-card-view-dialog.tsx` | 区分附件链接（indigo+硬盘图标）和文件链接（amber+文件图标），外部链接前缀 ExternalLink 图标，404 时显示失效提示 |
| 孤立附件清理 | `app/api/cards/note/route.ts` → `cleanupOrphanNoteImages()` | POST 后异步清理未引用的 `note-image`/`note-file` + 未关联/关联笔记已删除的 `note-attachment` |
>
...
| 方法 | 路径 | 说明 |
|:-----|:-----|:-----|
| `POST` | `/api/user/data/export` | 导出当前用户的标签、站点和资源（站点图标）为 ZIP（scope: full/sites-only） |
...
| `POST` | `/api/cards/note` | 创建笔记卡片（需认证，支持 `attachmentIds` 关联附件） |
| `PUT` | `/api/cards/note` | 更新笔记卡片（需认证，支持 `attachmentIds` 关联 + `softDeleteAttachmentIds` 软删除附件） |
| `DELETE` | `/api/cards/note?id=xxx` | 删除单张笔记卡片（需认证，同时清理关联附件文件） |
| `DELETE` | `/api/cards/note` | 删除全部笔记卡片（需认证，同时清理所有笔记附件文件） |
...
| `GET` | `/api/cards/note/attach/[attachId]` | 下载笔记附件（公开，原始文件名，kind=`note-attachment`） |
| `GET` | `/api/cards/note/attachment?noteId=xxx` | 获取指定笔记的附件列表（需认证） |
| `POST` | `/api/cards/note/attachment` | 上传笔记附件（需认证，FormData，最大 100MB，危险扩展名黑名单） |
| `PUT` | `/api/cards/note/attachment` | 重命名附件（需认证） |
| `DELETE` | `/api/cards/note/attachment?id=xxx` | 删除附件（需认证，物理删除文件+数据库记录） |
...
> 💡 **附件延迟持久化** — PUT 请求支持 `attachmentIds`（关联新附件到笔记）和 `softDeleteAttachmentIds`（软删除标记删除的附件，将 `note_id` 置空保留文件），配合撤销系统实现附件操作的可恢复性。附件上传时始终不关联 `noteId`，保存时统一处理。
...
| `POST` | `/api/snapshots` | 创建快照（label 可选，默认自动生成版本号 vN） |
| `DELETE` | `/api/snapshots?id=xxx` | 删除单个快照 |
| `PATCH` | `/api/snapshots?id=xxx` | 重命名快照 `{ label: string }` |
| `POST` | `/api/snapshots?action=restore&id=xxx` | 恢复快照（事务内清空当前数据并恢复快照内容，删除后续快照） |
| `POST` | `/api/snapshots?action=cleanup` | 清理过期快照（仅管理员，删除超过 30 天的快照） |
...
```json
{ "items": [{ "id": "snap-xxx", "ownerId": "__admin__", "label": "v1", "createdAt": "2026-05-04T12:00:00.000Z" }] }
```
...
**PATCH /api/snapshots?id=xxx（重命名）**

...
│   │       │   │   ├── attach/      # 笔记附件下载（大文件，kind='note-attachment'）
│   │       │   │   ├── attachment/  # 笔记附件管理（上传/列表/重命名/删除，最大 100MB）
...
│   │   │   ├── note-card-editor.tsx       # 笔记卡片编辑器（双 Tab：内容编辑/预览 + 附件管理，支持列表自动续行、剪贴板粘贴、/ 快捷指令、独立 Ctrl+Z 撤回、延迟持久化附件操作）
│   │   │   ├── note-attachment-tab.tsx     # 笔记附件管理标签页（独立组件，通过 ref 暴露 uploadSingleFile 供快捷指令调用）
...
| `logo` | Logo |
| `favicon` | Favicon |
| `note-image` | 笔记内联图片（剪贴板粘贴，≤5MB） |
| `note-file` | 笔记内联文件（剪贴板粘贴，≤10MB） |
| `note-attachment` | 笔记大文件附件（附件管理标签页上传，≤100MB，通过 `note_id` 关联笔记） |
...
> | 编辑器 / 快捷指令 | `components/sakura-nav/note-card-editor.tsx` | 输入 `/` 触发悬浮菜单，支持 todo/code/link/table 模板插入和 image/file 文件上传（小文件内联、大文件自动走附件、超大文件错误提示） |
> | 编辑器独立撤回 | `components/sakura-nav/note-card-editor.tsx` | Ctrl+Z 独立撤回栈（与全局撤回系统无关，最多 30 条，500ms 防抖） |
| 附件管理标签页 | `components/sakura-nav/note-attachment-tab.tsx` | 独立组件，双 Tab 布局中的附件管理 Tab（上传/下载/重命名/删除/复制引用），通过 `forwardRef` + `useImperativeHandle` 暴露 `uploadSingleFile` 供快捷指令调用 |
| 附件管理 API | `app/api/cards/note/attachment/route.ts` | 附件 CRUD（上传/列表/重命名/删除，100MB 限制，危险扩展名黑名单） |
| 附件下载（伪装 URL） | `app/api/cards/note/attach/[attachId]/route.ts` | `/api/cards/note/attach/{id}` 原始文件名下载，Content-Disposition |
| 查看弹窗附件渲染 | `components/dialogs/note-card-view-dialog.tsx` | 区分附件链接（indigo+硬盘图标）和文件链接（amber+回形针图标），404 时显示失效提示 |

...
│   │   │   ├── note-card-editor.tsx       # 笔记卡片编辑器（双 Tab：内容编辑/预览 + 附件管理，支持列表自动续行、剪贴板粘贴、延迟持久化附件操作）
...
| 附件管理标签页 | `components/sakura-nav/note-card-editor.tsx` | 双 Tab 布局中的附件管理 Tab（上传/下载/重命名/删除/复制引用） |
